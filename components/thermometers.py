...
class Thermometers:
    ...
    async def __extract_themperature(self, thermometer):
        themperature = None
        # b_line = self.__file_system.read_binary_file(thermometer['file_path'])
        b_line = await self.__file_system.aio_read_binary_file(thermometer['file_path'])
    ...
    async def measure_themperature(self):
        for i in range(len(self.__thermometers)):
            self.__thermometers[i]['value'] = await self.__extract_themperature(self.__thermometers[i])
